...
"""
function smstats_read_mwcs(shorttimestackdir::String, fodir::String, starttime::Union{String, DateTime}, endtime::Union{String, DateTime};
    ...
    df_mapped = pmap(x -> map_get_monitoringdf_mwcs(x, starttime, endtime), paths)
    ...
    df_all = DataFrame(date=DateTime[], stationpair=String[], networks=String[], components=String[], freqband=String[],
                        dvv_mwcs=Float64[], dvv0_mwcs=Float64[], dvv_err_mwcs=Float64[], dvv0_err_mwcs=Float64[])
...
function map_get_monitoringdf_mwcs(path::String, starttime::DateTime, endtime::DateTime)
    ...
    for timekey in keys(fi)
        for freqkey in keys(fi[timekey])
            ...
            # this file has overlap with the target timewindow [starttime, endtime]
                ...
                # append dvv
                if haskey(C.misc, "dvv_mwcs")
                    df_dvv = DataFrame(date = C.misc["stack_centraltime"], dvv_mwcs = float(C.misc["dvv_mwcs"]), dvv0_mwcs = float(C.misc["dvv0_mwcs"]),
                                        dvv_err_mwcs = float(C.misc["dvv_err_mwcs"]), dvv0_err_mwcs = float(C.misc["dvv0_err_mwcs"]))
